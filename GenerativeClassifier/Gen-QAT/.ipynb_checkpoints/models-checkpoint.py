...
class Gen(nn.Module):
    def __init__(self, ntoken, ninp, nlabelembed, nhid, nlayers, nclass, dropout, use_cuda, 
        ...
        self.encoder = qnn.QuantEmbedding(ntoken, ninp, weight_quant=Int8WeightPerTensorFixedPoint, weight_bit_width=bit_width)
        if one_hot:  # one_hot
            self.label_encoder = self.get_one_hot
        else:
            self.label_encoder = qnn.QuantEmbedding(nclass, nlabelembed, weight_quant=Int8WeightPerTensorFixedPoint, weight_bit_width=bit_width)
        if use_bias:
            self.bias_encoder = qnn.QuantEmbedding(nclass, ntoken, weight_quant=Int8WeightPerTensorFloat, weight_bit_width=bit_width)
        ...
        if concat_label != 'input':
            if one_hot:
                self.decoder = qnn.QuantLinear((nhid + nclass), ntoken, bias=False, weight_quant = Int8WeightPerTensorFixedPoint, weight_bit_width=bit_width)
            else:
                self.decoder = qnn.QuantLinear((nhid + nlabelembed), ntoken, bias=False, weight_quant = Int8WeightPerTensorFixedPoint, weight_bit_width=bit_width)
        else:
            self.decoder = qnn.QuantLinear(nhid, ntoken, bias=False, weight_quant = Int8WeightPerTensorFixedPoint, weight_bit_width=bit_width)
    ...
    def forward(self, x, x_pred, y_ext, hidden, criterion, is_infer=False):
        ...
        if is_infer:
            #print("Packed Sequence: ", nn.utils.rnn.PackedSequence(loss, x.batch_sizes))
            #print("Pad_Packed Sequence", nn.utils.rnn.pad_packed_sequence(nn.utils.rnn.PackedSequence(loss, x.batch_sizes)))
...
class Gen_2bit(nn.Module):
    def __init__(self, ntoken, ninp, nlabelembed, nhid, nlayers, nclass, dropout, use_cuda, 
        tied, use_bias, concat_label, avg_loss, one_hot, bit_width):
        
        super(Gen_2bit, self).__init__()
...
class Gen_1bit(nn.Module):
    def __init__(self, ntoken, ninp, nlabelembed, nhid, nlayers, nclass, dropout, use_cuda, 
        tied, use_bias, concat_label, avg_loss, one_hot, bit_width):
        
        super(Gen_1bit, self).__init__()
